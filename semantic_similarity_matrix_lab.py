...
<style>
:root{--bg:#07111f;--panel:#12233b;--line:#315478;--text:#eef5ff;--muted:#9db2ca;--cyan:#73dcff;--orange:#ed985b;--red:#ff8295}
*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font-family:Inter,system-ui,sans-serif}.wrap{max-width:1600px;margin:auto;padding:24px}header{background:#0b192b;border-bottom:1px solid var(--line)}h1,h2{margin-top:0}.card{background:var(--panel);border:1px solid var(--line);border-radius:16px;padding:20px;margin-top:18px}label{display:block;color:var(--cyan);margin:8px 0 5px}textarea,select{width:100%;padding:12px;border-radius:9px;border:1px solid #41658b;background:#071522;color:var(--text)}textarea{min-height:190px;font-family:Consolas,monospace}button{margin-top:16px;padding:12px 18px;border:0;border-radius:9px;background:var(--orange);color:white;font-weight:800;cursor:pointer}.grid{display:grid;grid-template-columns:2fr 1fr 1fr;gap:18px}.two{display:grid;grid-template-columns:1fr 1fr;gap:18px}p{color:var(--muted)}table{border-collapse:collapse;width:100%}th,td{padding:9px;border:1px solid var(--line);text-align:center;white-space:nowrap}th.sticky{position:sticky;left:0;background:#102139;z-index:2;text-align:left}.scroll{overflow:auto}.cell{font-weight:800}.bad{color:var(--red)}@media(max-width:950px){.grid,.two{grid-template-columns:1fr}}
</style>
</head>
<body>
...
<form method="post">
<div class="grid">
<div><label>入力（1行に 主体｜関係｜内容）</label><textarea name="items" required>{{ items }}</textarea></div>
<div><label>観測段階</label><select name="stage">
<option value="subject"{% if stage == 'subject' %} selected{% endif %}>主体</option>
<option value="relation"{% if stage == 'relation' %} selected{% endif %}>関係</option>
<option value="content"{% if stage == 'content' %} selected{% endif %}>内容</option>
</select></div>
<div><label>類似度指標</label><select name="metric">
<option value="activation"{% if metric == 'activation' %} selected{% endif %}>最終活性</option>
<option value="nodes"{% if metric == 'nodes' %} selected{% endif %}>累積Node</option>
<option value="edges"{% if metric == 'edges' %} selected{% endif %}>累積Edge</option>
</select></div>
</div>
<button>行列を作成する</button>
</form>
<p>learn=False・noise=0。CoreとDBは変更しません。最大14件です。</p>
</section>
