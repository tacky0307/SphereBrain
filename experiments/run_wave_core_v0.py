...
def trace_metrics(trace, core: SphereWaveCore, b_region: tuple[int, ...]) -> dict[str, Any]:
    ...
    centers = np.asarray([snapshot.center for snapshot in trace.snapshots], dtype=float)
    totals = np.asarray(
        [snapshot.total_activity for snapshot in trace.snapshots], dtype=float
    )
    b_means = np.asarray(
        [float(np.mean(snapshot.activity[b_ids])) for snapshot in trace.snapshots],
        dtype=float,
    )
    active_counts = [
    ...
    return {
        ...
        "total_firing_events": int(
            sum(len(snapshot.fired_nodes) for snapshot in trace.snapshots)
        ),
        "center_path_length": path_length,
        "closest_center_distance_to_b": float(
            np.min(np.linalg.norm(centers - target[None, :], axis=1))
        ),
...
def save_trace_csv(
    ...
    with path.open("w", newline="", encoding="utf-8-sig") as handle:
        ...
        for snapshot in trace.snapshots:
            b_values = snapshot.activity[b_ids]
    ...
    with path.open("w", newline="", encoding="utf-8-sig") as handle:
        ...
        for snapshot in trace.snapshots:
            ...
            for node_id, (activity, fatigue) in enumerate(
                ...
                writer.writerow(
                    [snapshot.step, node_id, float(activity), float(fatigue), int(node_id in fired)]
                )
    ...
    payload = json.dumps(
        {
            "result": result,
            "baselineSeries": [
                {"step": item.step, "activity": item.total_activity}
...
<title>__TITLE__ Observation Report</title>
<style>
...
const fmt = value => Number.isFinite(Number(value)) ? Number(value).toPrecision(6) : String(value);
const cards = [
...
def main() -> None:
    ...
    terrain_rows = sorted(
        (
            (
                int(source),
                int(target),
                float(terrain_before[source, target]),
                float(core.conductivity[source, target]),
                float(terrain_delta[source, target]),
            )
    ...
    result = {
        ...
        "repetitions": args.repetitions,
        "delay_steps": args.delay_steps,
        "config": vars(core.config),
        "baseline": baseline_metrics,
        "trained": trained_metrics,
        "change": {
            "b_region_peak": trained_metrics["b_region_peak"] - baseline_metrics["b_region_peak"],
            "b_region_activity_integral": trained_metrics["b_region_activity_integral"] - baseline_metrics["b_region_activity_integral"],
            "center_distance_to_b": trained_metrics["closest_center_distance_to_b"] - baseline_metrics["closest_center_distance_to_b"],
            "activity_integral": trained_metrics["activity_integral"] - baseline_metrics["activity_integral"],
            "center_path_length": trained_metrics["center_path_length"] - baseline_metrics["center_path_length"],
    ...
    render_observation_report(
        output / "observation_report.html", result, baseline, trained
    )
