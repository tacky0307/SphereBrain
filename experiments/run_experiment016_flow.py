...
def main() -> None:
    parser = argparse.ArgumentParser(description="Experiment 016: Experience-guided Flow")
    ...
    parser.add_argument("--bias-gain", type=float, default=0.30)
    parser.add_argument("--bias-max", type=float, default=1.50)
    parser.add_argument("--memory-gain", type=float, default=0.05)
    parser.add_argument("--memory-decay", type=float, default=0.995)
    ...
    output = Path("results") / "experiment_016"
    rows: list[dict[str, object]] = []

    print("Experiment 016: Experience-guided Flow")
    print("training: A -> P and F -> P")
    print("comparison: local plasticity vs experience-guided flow")
    ...
    for brain in range(1, args.brains + 1):
        ...
        field_config = ExperienceFieldConfig(
            ...
            propagation_bias_gain=args.bias_gain,
            propagation_bias_max=args.bias_max,
    ...
    write_csv(output / "experience_guided_flow_curve.csv", rows)
    final = [
        row for row in rows
