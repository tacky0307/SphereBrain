...
def train_pair(
    ...
    """Train one source -> destination experience with an explicit boundary.

    Learning occurs only during the source and destination phases. The experience
    is closed immediately after the destination phase, and the following settling
    phase is always non-learning for both field and local controls.
    """
    core.reset_activity()
    if use_field:
    ...
    learned_history: list[np.ndarray] = []

    core.stimulate(source, 1.0)
    for _ in range(5):
        learned_history.append(core.step(learn=True).activity.copy())

    core.stimulate(destination, 1.0)
    for _ in range(7):
        learned_history.append(core.step(learn=True).activity.copy())

    experience_strength_sum = 0.0
    experience_strength_mean = 0.0
    if use_field:
        field = core.end_experience()
        experience_strength_sum = float(np.sum(field))
        experience_strength_mean = float(np.mean(field))

    # State may continue evolving, but the experience and its learning are over.
    settle_history: list[np.ndarray] = []
    for _ in range(8):
        settle_history.append(core.step(learn=False).activity.copy())

    history = learned_history + settle_history
    return (
        np.mean(np.stack(history, axis=0), axis=0),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Experiment 014.2: Experience Boundary Learning")
    ...
    parser.add_argument("--trace-gain", type=float, default=1.0)
    ...
    output = Path("results") / "experiment_014_2_boundary_learning"
    rows: list[dict[str, object]] = []
    training_rows: list[dict[str, object]] = []

    print("Experiment 014.2: Experience Boundary Learning")
    print("training: A -> P and F -> P")
    print("learning boundary: source + destination only")
    print("post-experience settling: 8 steps with learn=False")
    ...
    for brain in range(1, args.brains + 1):
        ...
        latest_strength = {"A": (0.0, 0.0), "F": (0.0, 0.0)}
        ...
        for repetition in range(args.repetitions + 1):
            if repetition in checkpoints:
                ...
                for (mode, cue), (pattern, metrics) in results.items():
                    ...
                    strength_sum, strength_mean = (
                        latest_strength[cue] if mode == "field" and cue in ("A", "F") else (0.0, 0.0)
                    )
                    rows.append(
                        {
                            "brain": brain,
                            ...
                            **metrics,
                            ...
                            "experience_strength_sum": strength_sum,
                            "experience_strength_mean": strength_mean,
            ...
            if repetition < args.repetitions:
                ...
                for cue in order:
                    local_pattern, _, _ = train_pair(local, regions[cue], regions["P"], False)
                    field_pattern, strength_sum, strength_mean = train_pair(
                        field, regions[cue], regions["P"], True
                    )
                    latest[f"local_{cue}"] = local_pattern
                    latest[f"field_{cue}"] = field_pattern
                    latest_strength[cue] = (strength_sum, strength_mean)
    ...
    write_csv(output / "experience_boundary_learning_curve.csv", rows)
    write_csv(output / "experience_strength_by_repetition.csv", training_rows)

    final = [
        row for row in rows
        if row["checkpoint"] == args.repetitions and row["mode"] == "field"
    ]
    print("final experience-boundary results:")
    for row in final:
        print(
            ...
            f"experience strength={float(row['experience_strength_sum']):.12g}"
        )
