...
def build_dormant_brain() -> Top50SharedExperienceRecoveryBrain:
...
def recovery_train(brain, input_encoder, output_encoder, examples, epochs):
    ...
    for epoch in range(epochs):
        if isinstance(brain, Top50SharedExperienceRecoveryBrain):
        ...
        if isinstance(brain, Top50SharedExperienceRecoveryBrain):
            ...
            print(
                ...
                f"same_input_ignored={int(measured['duplicate_input_selections_ignored'])}"
            )
...
def print_state_stats(brain: DormantSurfaceFlowBrain, label: str) -> None:
    ...
    if isinstance(brain, Top50SharedExperienceRecoveryBrain):
        ...
        print(
            ...
            f"shared_two_or_more={int(measured['candidate_paths_two_or_more_distinct_inputs'])} "
            f"max_inputs_same_region={int(measured['max_distinct_inputs_same_region'])}"
        )
        print(
            f"eligible_by_region: low={int(measured['low_region_promotion_eligible'])} "
...
def main() -> None:
    base.DiverseRegionCappedRecoveryBrain = Top50SharedExperienceRecoveryBrain
    base.build_dormant_brain = build_dormant_brain
    base.recovery_train = recovery_train
    base.print_state_stats = print_state_stats

    print("SphereBrain top50 shared-experience dormant recovery experiment v11")
    print("task: y = 2x")
    print(
    ...
    print(f"actual candidate width: top {ACTUAL_CANDIDATE_WIDTH}")
    ...
    print("teacher reinforcement only after promotion")
    ...
    base.run_condition(
        "top50 shared-experience dormant recovery v11",
        build_dormant_brain(),
    )
